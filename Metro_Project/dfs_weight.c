...
void dfs_weight_recurrence(int st, int s, int e, int (*m)[197], int (*crd)[8], int amount, double cost, Way *dw, Station *ds, int *b1, int *a1, double *c1, int *w1, double *wt1, int *b2, int *a2, double *c2, int *w2, double *wt2, int *b3, int *a3, double *c3, int *w3, double *wt3, double wt, int *nowW)
{
    int i;
    if (s == e)
    {
    ...
    for (i = 0; i < 197; i++)
    {
        if ((i != s) && (m[s][i] != 0) && (visited[i] == 0))
        {
            ...
            if (amount == 1)
            {
                ...
                dfs_weight_recurrence(st, i, e, m, crd, amount + 1, cost + 3.0, dw, ds, b1, a1, c1, w1, wt1, b2, a2, c2, w2, wt2, b3, a3, c3, w3, wt3, (wt + rw), nowW);
            }
            ...
            else if ((amount > 1) && (nowLine[amount] != nowLine[amount - 1]))
            {
                ...
                dfs_weight_recurrence(st, i, e, m, crd, amount + 1, cost + 6.5, dw, ds, b1, a1, c1, w1, wt1, b2, a2, c2, w2, wt2, b3, a3, c3, w3, wt3, (wt + rw), nowW);
            }
            visited[i] = 0;
        }
    }
...
void dfs_weight(int d_s_num, int d_e_num, Way *d_w, Station *d_s, int (*m)[197], int (*crd)[8], int st)
{
    ...
    dfs_weight_recurrence(st, d_s_num, d_e_num, m, crd, 1, 0.0, d_w, d_s, b1, &a1, &c1, w1, wt1, b2, &a2, &c2, w2, wt2, b3, &a3, &c3, w3, wt3, 0, nowW);
